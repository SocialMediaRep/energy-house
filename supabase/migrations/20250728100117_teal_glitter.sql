/*
  # Add remaining devices to the devices table

  1. New Devices
    - Adds all missing devices from the original device list
    - Includes devices for all rooms: bathroom, bedroom, living, kitchen, garage, basement
    - Each device has complete information: wattage, standby, tips, descriptions

  2. Data Integrity
    - Uses INSERT ... ON CONFLICT DO NOTHING to avoid duplicates
    - Maintains existing device states if already present
*/

-- Insert all remaining devices (will skip if they already exist)
...
-- Wohnzimmer (Router bereits vorhanden, andere hinzufügen)
...
-- Küche (Kühlschrank bereits vorhanden, andere hinzufügen)
